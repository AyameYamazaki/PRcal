...
fileno = 12
eye = "right"
dp_threshold = 2 #3 if fileno 1 or 7

if(fileno==1 & dp_threshold==2){
  stop("ARE YOU SURE YOU WANT TO KEEP THE DP_THRESHOLD TO 2?")
}
if(fileno==7 & dp_threshold==2){
    stop("ARE YOU SURE YOU WANT TO KEEP THE DP_THRESHOLD TO 2?")
}
...
highpass=20 #change the passband if noisy***************************************************************************
lowpass=-15 #change the passband if noisy***************************************************************************
...
dp_threshold = 3 #change threshold according to data ################################################################
...
library(openxlsx)
output_list <- list("Listing's Plane"=LP,  "rawdata"=rt.data, "Fick"=fick, "Rotation"=rotation, "RotationNR"=rotation_outremoved, "DPnormalvector"=RVector, "Primary Based"=PrimaryBased, "PrimaryBasedNR"=primary_recurrence, "Primary position"=Primary_position)
write.xlsx(output_list, paste("result_turntable_" , per_post, "_9pt_", which_side, "_", fileno, ".xlsx", sep = ""), overwrite = TRUE)

pdf(paste("2dplots_turntable_" , per_post, "_9pt_", which_side, "_", fileno, ".pdf", sep = ""), width = 10, height = 10,  bg="white")
print(yx)
print(yz)
print(xz)
print(yx_lp)
...
pdf(paste("rawplot_turntable_", per_post, "_9pt_", which_side, "_", fileno, ".pdf", sep = ""), width = 10, height = 10, bg="white")
plot_eyerayx
plot_eyerayy
plot_eyerayz
plot_torsion
...
pdf(paste("3dplots_turntable_", per_post, "_9pt_", which_side, "_", fileno, ".pdf", sep = ""), width = 10, height = 10, bg="white")
s3d <- scatterplot3d(rotation$x3, rotation$y3,rotation$z3, pch=20, color = "blue", type="p", lty.hplot=2)
s3d$points3d(rotation_outremoved$x3, rotation_outremoved$y3, rotation_outremoved$z3, col="green", pch=20)
s3d$points3d(rbind(centroid, centroid + normal_vector), type = "l", col = "red", lwd = 2)
s3d_LP <- scatterplot3d(primary_recurrence_nonomitted$rotvecdegx, primary_recurrence_nonomitted$rotvecdegy, primary_recurrence_nonomitted$rotvecdegz, color="blue",  pch=20, type="p", lty.hplot=2)
