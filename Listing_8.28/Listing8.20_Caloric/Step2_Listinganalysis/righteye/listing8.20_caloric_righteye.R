...
ear = "left"
fileno = 18      #ここでファイル番号を適宜へんこうしていく。出力される結果もこの番号になる。
band_sd = 2.5 #torsionのSD値を変更（なるべく５以下にはしないように）
...
setting <- data.frame(band_sd)

library(openxlsx)
output_list <- list("Listing's Plane"=LP,  "rawdata"=rt.data, "Fick"=fick, "Rotation"=rotation, "RotationNR"=rotation_outremoved, "DPnormalvector"=RVector, "Primary Based"=PrimaryBased, "PrimaryBasedNR"=primary_recurrence, "Primary position"=Primary_position, "setting"=setting)
