...
#do not change below 2.5
band_sd = 5



#change the parameters accordingly***********************************************************************
paste("caloric_", "9pt_", which_side, "_", fileno, ".csv", sep="")
...
if(l>(w_erx*2+1)){
  for(t in (w_erx+1):(l-w_erx)){
    ...
    Ut_erx<-erx[(t-w_erx):(t+w_erx)]#eye ray x within the window
        if(abs(erx[t]-mean(Ut_erx))>k_erx*sd(Ut_erx)) { #outlier if k_erx * standard deviation away from the mean within the window
      df_erx$outlier_erx[t] <- TRUE 
      df_erx$presmooth_erx[t] <- mean(Ut_erx)　#replace outlier with mean of the window
    }
  }
}
...
df_eyerayx$erxNR_PB <- if_else(df_eyerayx$erxNR>-0.3&df_eyerayx$erxNR<0.3, df_eyerayx$erxNR, NaN)
...
#function for converting degrees to radian
radian <- function(x){
  rad <-  x * pi / 180
  return(rad)
}
...
# combine all intrinsic rotations into a quaternion
rotation <- fick %>% 
...
#regression plane using x=-ay-bz-c
dp_threshold = 10 # do not change

lm <- lm(x3 ~ y3 + z3, rotation, na.action=na.omit) 
inv_l_DP <- c(lm$coefficients)
#since x=-ay-bz-c reverse to minus
l_DP <- -(inv_l_DP)
names(l_DP) <- c("c", "a", "b")

#removing outliers on three dimensional plane (inactivated with high dp_threshold)
...
#calculate arctanz/x, elevation of LP
...
#rotate displacement plane so that primary position aligns with reference position
...
write.xlsx(output_list, paste("result_caloric_" , "9pt_", which_side, "_", fileno, ".xlsx", sep = ""), overwrite = TRUE)

pdf(paste("2dplots_caloric_" , "9pt_", which_side, "_", fileno, ".pdf", sep = ""), width = 10, height = 10,  bg="white")
print(yx)
print(yz)
print(xz)
print(yx_lp)
...
pdf(paste("rawplot_caloric_", "9pt_", which_side, "_", fileno, ".pdf", sep = ""), width = 10, height = 10, bg="white")
plot_eyerayx
plot_eyerayy
plot_eyerayz
plot_torsion
...
pdf(paste("3dplots_caloric_", "9pt_", which_side, "_", fileno, ".pdf", sep = ""), width = 10, height = 10, bg="white")
s3d <- scatterplot3d(rotation$x3, rotation$y3,rotation$z3, pch=20, color = "blue", type="p", lty.hplot=2)
s3d$points3d(rotation_outremoved$x3, rotation_outremoved$y3, rotation_outremoved$z3, col="green", pch=20)
s3d$points3d(rbind(centroid, centroid + normal_vector), type = "l", col = "red", lwd = 2)
s3d_LP <- scatterplot3d(primary_recurrence_nonomitted$rotvecdegx, primary_recurrence_nonomitted$rotvecdegy, primary_recurrence_nonomitted$rotvecdegz, color="blue",  pch=20, type="p", lty.hplot=2)
