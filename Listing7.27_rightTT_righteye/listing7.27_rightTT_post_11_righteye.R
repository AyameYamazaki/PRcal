...
data <- read.csv("turntable_post_9pt_right_11.csv") #csvファイルを読み込む make sure it is a control file! **********************

which_side = "right" #rotation or caloric ear**********************************************************************
...
dp_threshold = 1.5 #change threshold according to data ################################################################
...
lp_sd = 4 #change threshold according to data ******************************************************************************
...
library(openxlsx)
output_list <- list("Listing's Plane"=LP, "Fick"=fick, "Rotation"=rotation, "RotationNR"=rotation_outremoved, "DPnormalvector"=RVector, "Primary Based"=PrimaryBased, "PrimaryBasedNR"=primary_recurrence, "Primary position"=Primary_position)
write.xlsx(output_list, paste("result_turntable_post_9pt_right_11.xlsx", sep = ""), overwrite = TRUE)

pdf(paste("2dplots_turntable_post_9pt_right_11.pdf", sep = ""), width = 10, height = 10)
yx_b <- ggplot()+
  geom_point(data=rotation_outremoved, aes(x =y3, y = x3), color="black", size=0.5)+
  labs(title = "DP_yx", x = "y", y = "x")+
  xlim(-20, 20) +
  ylim(-20, 20)
yz_b <- ggplot()+
  geom_point(data=rotation_outremoved, aes(x =y3, y = z3), color="black", size=0.5)+
  labs(title = "DP_yz", x = "y", y = "z")+
  xlim(-20, 20) +
  ylim(-20, 20)
xz_b <- ggplot()+
  geom_point(data=rotation_outremoved, aes(x =x3, y = z3), color="black", size=0.5)+
  labs(title = "DP_xz", x = "x", y = "z")+
  xlim(-20, 20) +
  ylim(-20, 20)

yx_lp_b <- ggplot()+
  geom_point(data=primary_recurrence, aes(x =rotvecdegy, y = rotvecdegx), color="black", size=0.5)+
  labs(title = "LP_yx", x = "y", y = "x")+
  xlim(xlim_low, xlim_upp) +
  ylim(ylim_low, ylim_upp)
yz_lp_b <- ggplot()+
  geom_point(data=primary_recurrence, aes(x =rotvecdegy, y = rotvecdegz), color="black", size=0.5)+
  labs(title = "LP_yz", x = "y", y = "z")+
  xlim(xlim_low, xlim_upp) +
  ylim(ylim_low, ylim_upp)
xz_lp_b <- ggplot()+
  geom_point(data=primary_recurrence, aes(x =rotvecdegx, y = rotvecdegz), color="black", size=0.5)+
  labs(title = "LP_xz", x = "x", y = "z")+
  xlim(xlim_low, xlim_upp) +
  ylim(ylim_low, ylim_upp)
print(yx_b)
...
pdf(paste("rawplot_turntable_post_9pt_right_11.pdf", sep = ""), width = 10, height = 10)
plot_eyerayx
plot_eyerayy
plot_eyerayz
plot_torsion
...
pdf(paste("3dplots_turntable_post_9pt_right_11.pdf", sep = ""), width = 10, height = 10)
s3d_b <- scatterplot3d(rotation_outremoved$x3, rotation_outremoved$y3, rotation_outremoved$z3, color="black", pch=20, type="p", lty.hplot=2)
s3d_b$points3d(rbind(centroid, centroid + normal_vector), type = "l", col = "red", lwd = 2)
s3d_LP_b <- scatterplot3d(primary_recurrence$rotvecdegx, primary_recurrence$rotvecdegy,primary_recurrence$rotvecdegz, color="black", pch=20, type="p", lty.hplot=2)
s3d_LP_b$points3d(rbind(centroid_LP, centroid_LP + normal_vectorLP), type = "l", col = "red", lwd = 2)
dev.off()
