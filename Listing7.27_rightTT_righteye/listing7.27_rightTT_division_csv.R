...
data2 <- read.csv(file=file.choose()) #csvファイルを読み込む make sure it is a control file! **********************
...
headdeg_stop <- headdeg %>% dplyr::filter(time >50 & time < 70)
...
stop_time <- min(headdeg_stopped$time) #check stop time is correct on the plot *****************************************
