...
#change to right post accordingly
which_side = "left"
per_post = "per" #is it at the beginning of rotation (per) or after rotation stops (post)
headpos = "30ND"
eye = "right"

#Use this script for 30ND leftper and rightpost, 30NU leftper and rightpost, and 60NU leftper and rightpost although check the raw data before determining which side


Z_timelimit = 0.05
Rolling_threshold = 5L 
...
for(i in 2:nrow(df_smoothz_vel)){
  if(df_smoothz_vel$rightsac[i]==TRUE){
    df_smoothz_vel$rightsacelev[i] <- df_smoothz_vel$smoothz[i]+correcting_threshold
  }else{
...
Z.ZdegLIST <- dplyr::filter(Z.result, bottomOrTop == 0) %>% 
...
setting <- data.frame(Rolling_threshold, Z_timelimit, eye)
