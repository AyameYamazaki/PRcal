...
#Smooth the Ydeg
Rolling_threshold = 21L #determine the window size for rolling average
...
for(i in 2:nrow(df_smoothY_vel)){
  if(df_smoothY_vel$downsac[i]==TRUE){
    df_smoothY_vel$downsacelev[i] <- df_smoothY_vel$smoothY[i]+3
  }else{
...
#SPEV抽出
Y.YdegLIST <- filter(Y.result, bottomOrTop == 0) %>% 
...
SPEVY_plot <- ggplot() + 
  labs(x="Time", y="Ydeg") + 
  geom_line(aes(x=NR_eyerays$time, y=NR_eyerays$smoothedY, col="Ydeg"), linetype="dashed", linewidth=0.5) + 
