...
paste("turntable_", per_post, "_9pt_", which_side, "_", fileno, ".csv", sep="")
data <- read.csv(paste("turntable_", per_post, "_9pt_", which_side, "_", fileno, ".csv", sep="")) 
...
#NR_eyerays$smoothedY <- savgol(NR_eyerays$Ydeg, 39)
...
#where vel <-30, there is upward saccade. Since the below script assumes upward saccade as part of the SPEV, we need to create a block so that SPEV calculation does not include the upward saccade.
upsac_threshold = 30 #determine threshold for detecting upward saccade
df_smoothY_vel$upsac <- rep(FALSE, nrow(df_smoothY_vel))
for(i in 2:nrow(df_smoothY_vel)){
  if(df_smoothY_vel$velY[i] > upsac_threshold){
    df_smoothY_vel$upsac[i] <- TRUE #TRUE if upwardsac is occurring
  }
}
...
df_smoothY_vel$upsacelev <- rep(0,nrow(df_smoothY_vel))
for(i in 2:nrow(df_smoothY_vel)){
  if(df_smoothY_vel$upsac[i]==TRUE){
    df_smoothY_vel$upsacelev[i] <- df_smoothY_vel$smoothY[i]-3
  }else{
    df_smoothY_vel$upsacelev[i] <- df_smoothY_vel$smoothY[i]
  }
}

Y_plot <- ggplot()+
  geom_line(data=NR_eyerays, aes(x=time, y=Ydeg, col="Ydeg")) +
  ...
  geom_line(data=df_smoothY_vel, aes(x=time, y=upsacelev, col="correctedupsac"))
  labs(title = "Y_degree", x = "time", y = "Ydeg")+
  scale_x_continuous(breaks=seq(0,10,by=1))+
  ylim(-15, 15)
Y_plot

#########check the Y_plot and if the quick phase is UPWARD IN THE PLOT (i.e. nystagmus is downward), do not use this script, but the  other one. ##################################################################
...
Ydeg <- na.omit(preYdeg$upsacelev)
...
#SPEV抽出
Y.YdegLIST <- filter(Y.result, bottomOrTop == 1) %>% 
...
library(openxlsx)
output_list <- list("Average_SPEVY"=Average_SPEVY, "top3_SPEVY"=result_SPEVY, "All_SPEVY"=Y.sorted.data, "Eyeray_data"=NR_eyerays)
