...
for(i in 2:nrow(df_smoothz_vel)){
  if(df_smoothz_vel$leftsac[i]==TRUE){
    df_smoothz_vel$leftsacelev[i] <- df_smoothz_vel$smoothz[i]-3
  }else{
...
Z_plot <- ggplot()+
  ...
  geom_line(data=df_smoothz_vel, aes(x=time, y=leftsacelev, col="correctedRsac"))
...
#SPEV抽出
Z.ZdegLIST <- filter(Z.result, bottomOrTop == 1) %>% 
