...
raw_data$smoothedtor <- rollapply(raw_data$torsion, width=Rolling_threshold, FUN=mean, partial=TRUE)
#raw_data$smoothedtor <- savgol(raw_data$torsion, 39)
df_smoothtor <- data.frame(raw_data$time, raw_data$smoothedtor)
...
for(i in 2:nrow(df_smoothtor_vel)){
  if(df_smoothtor_vel$upsac[i]==TRUE){
    df_smoothtor_vel$upsacelev[i] <- df_smoothtor_vel$smoothtor[i]-3
  }else{
...
tor_plot <- ggplot()+
  geom_line(data=raw_data, aes(x=time, y=torsion, col="torsion"))+
  geom_line(data=raw_data, aes(x=time, y=smoothedtor, col="smoothedtor Line"))+
  geom_point(data=raw_data, aes(x=time, y=smoothedtor, col="smoothedtor Point"),size=0.5)+
  geom_line(data=df_smoothtor_vel, aes(x=time, y=upsacelev, col="correctedupsac"))
  labs(title = "torsion_degree", x = "time", y = "torsion")+
...
tor.result <- data.frame(tor.index, tor.bottomOrTop) %>% 
  mutate(SPEVtime = raw_data$time[tor.index]) %>% 
  mutate(tor = raw_data$smoothedtor[tor.index])

#NAを除いたデータに上から順にindexが割り当てられているので、本来のindexに戻す。
tor.true.index <- array()
for (i in 1:nrow(tor.result)) {
  INDEX <- which(raw_data$time == tor.result$SPEVtime[i])  
...
#SPEV抽出
tor.tordegLIST <- filter(tor.result, bottomOrTop == 1) %>% 
...
SPEVtor_plot <- ggplot() + 
  ...
  geom_line(aes(x=raw_data$time, y=raw_data$smoothedtor, col="tordeg"), linewidth=0.5) + 
  ...
  scale_x_continuous(breaks=seq(floor(min(raw_data$time)),floor(max(raw_data$time)), 1))
SPEVtor_plot
...
library(openxlsx)
output_list <- list("Average_SPEVtor"=Average_SPEVtor, "top3_SPEVtor"=result_SPEVtor, "All_SPEVtor"=tor.sorted.data, "Eyeray_data"=raw_data)
