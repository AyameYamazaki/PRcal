...
cutNR <- NR_eyerays %>% dplyr::filter(time>1)

average_Y <- mean(cutNR$Ydeg, na.rm=TRUE)
result <- data.frame(average_Y, eye, which_side)
...
output_list <- list("result"=result, "Y_data"=NR_eyerays)
write.xlsx(output_list, paste("1ptturntable_result_Y_control_", which_side, ".xlsx", sep=""))