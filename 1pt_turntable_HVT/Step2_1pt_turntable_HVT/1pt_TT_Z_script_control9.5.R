...
#remove first 1sec as subject looks at different place first
cutNR <- NR_eyerays %>% dplyr::filter(time>1)

average_Z <- mean(cutNR$Zdeg, na.rm=TRUE)
result <- data.frame(average_Z, eye, which_side)
...
output_list <- list("result"=result, "Z_data"=NR_eyerays)
write.xlsx(output_list, paste("1ptturntable_result_Z_control_", which_side, ".xlsx", sep=""))