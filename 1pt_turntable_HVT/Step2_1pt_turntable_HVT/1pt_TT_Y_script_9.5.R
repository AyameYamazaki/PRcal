...
NR_eyerays$smoothedY <- rollapply(NR_eyerays$Ydeg, width=Rolling_threshold, FUN=mean, partial=TRUE, align="left")
...
Y_plot <- ggplot()+
  geom_line(data=NR_eyerays, aes(x=time, y=Ydeg, col="Ydeg"), linewidth=0.3) +
  geom_line(data=df_smoothY, aes(x=time, y=smoothY, col="smoothedY Line"), linewidth=1)+
labs(title = "Y_degree", x = "time", y = "Ydeg")+
  scale_x_continuous(breaks=seq(0,120,20))
Y_plot


pdf(paste("1ptturntable_Y_plot",  which_side, "_", pre_post, ".pdf", sep=""),  width = 15, height = 10)
print(Y_plot)
dev.off()


output_list <- list("Y_data"=NR_eyerays, "setting"=setting)
write.xlsx(output_list, paste("1ptturntable_Y_result_",  which_side, "_", pre_post, ".xlsx", sep=""))
