...
data <- read.csv(file=file.choose()) #choose control!!
...
df_torsion$smoothedtor <- rollapply(df_torsion$torNR_PB, width=Rolling_threshold, FUN=mean, partial=TRUE, na.rm=TRUE, align="center")
df_smoothtor <- data.frame(df_torsion$time, df_torsion$smoothedtor)
colnames(df_smoothtor) <- c("time", "smoothtor")
#remove first 0.8sec as subject looks at different place first
df_smoothtor <- df_smoothtor %>% dplyr::filter(time>1)

average_tor <- mean(df_smoothtor$smoothtor, na.rm=TRUE)
result <- data.frame(average_tor, eye, which_side)
...
output_list <- list("result"=result, "tor_data"=df_torsion)
write.xlsx(output_list, paste("1ptturntable_result_tor_control_", which_side, ".xlsx", sep=""))